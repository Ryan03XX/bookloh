...
/** fast-glob requires forward slashes (path.join on Windows breaks matches). */
...
const createAssetBlacklistPattern = (type) => {
    ...
    const filePatterns = config.files.map(file => '!' + globPath(SRC_FOLDER, 'assets', type, '**', file));
    const folderPatterns = config.folders.map(folder => '!' + globPath(SRC_FOLDER, 'assets', type, folder));
...
async function copyAssets() {
    ...
    await copyFiles(
        [
            globPath(SRC_FOLDER, 'assets/fonts/**/*'),
    ...
    async function copyFirstFound(fileName) {
        ...
        for (const srcPath of candidates) {
            try {
                ...
                await fs.copyFile(srcPath, path.join('dist', fileName));
                return path.relative(__dirname, srcPath);
            } catch {
                // try next
            }
        }
    ...
    const videoSources = [
        path.join(SRC_FOLDER, 'main_compress.mp4'),
        path.join(__dirname, 'main_compress.mp4'),
        path.join(SRC_FOLDER, 'main.mp4'),
        path.join(__dirname, 'main.mp4'),
    ];
    for (const srcPath of videoSources) {
        try {
            await fs.access(srcPath);
            await fs.copyFile(srcPath, path.join('dist', videoDest));
            console.log('📹 Hero video:', path.relative(__dirname, srcPath), '→ dist/' + videoDest);
            break;
        } catch {
            // try next
        }
    ...
    // Brand icons under src/assets/icons (HTML: ../assets/icons/...)
    const iconsDir = path.join(SRC_FOLDER, 'assets/icons');
    try {
        await fs.access(iconsDir);
        await copyFiles(
            [globPath(SRC_FOLDER, 'assets/icons/**/*')],
            'dist/assets/icons',
            { base: iconsDir }
        );
    } catch {
    ...
    // Optional: src/icons/ (flat) merged into dist/assets/icons so paths match HTML
    const flatIconsDir = path.join(SRC_FOLDER, 'icons');
    try {
        await fs.access(flatIconsDir);
        await copyFiles(
            [globPath(SRC_FOLDER, 'icons/**/*')],
            'dist/assets/icons',
            { base: flatIconsDir }
        );
    } catch {
    ...
    try {
        ...
        await copyFiles(
            [
                globPath(SRC_FOLDER, 'assets/images/**/*'),
...
async function processCSS() {
    ...
    const cssFiles = await glob([
        globPath(SRC_FOLDER, 'assets/css/**/*.css'),
...
async function processSass() {
    ...
    const files = await glob(globPath(SRC_FOLDER, 'assets/scss/theme/*.scss'));
...
async function processHTML() {
    ...
    const files = await glob(globPath(SRC_FOLDER, '**/*.html'));
    
    for (const file of files) {
        let content = await fs.readFile(file, 'utf8');
        // Map theme SCSS names to compiled CSS; do not turn main.min.css → main.min.min.css
        content = content.replace(
            /\.\.\/assets\/css\/theme\/([\w-]+)(?:\.min)?\.css/g,
            (_match, baseName) =>
                `../assets/css/theme/${baseName}${usePurgeCSS ? '.purge' : '.min'}.css`
        );
...
async function processJS() {
    ...
    const jsFiles = await glob([
        globPath(SRC_FOLDER, 'assets/js/**/*.js'),
    ...
    const jsCssFiles = await glob([
        globPath(SRC_FOLDER, 'assets/js/**/*.css'),
